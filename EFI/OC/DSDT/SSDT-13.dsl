 ...
 *
 * Disassembly of SSDT-13.aml, Sun Feb  6 20:50:25 2022
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x000038DC (14556)
 *     Revision         0x01
 *     Checksum         0xF6
 *     OEM ID           "HPQOEM"
 *     OEM Table ID     "INSYDE  "
...
DefinitionBlock ("", "SSDT", 1, "HPQOEM", "INSYDE  ", 0x00001000)
{
    ...
    External (_SB_.PCI0.LPCB.EC0_.GCTM, UnknownObj)    // (from opcode)
    External (_SB_.PCI0.RP01, DeviceObj)    // (from opcode)
    External (_SB_.PCI0.RP01.PXSX, DeviceObj)    // (from opcode)
    External (_SB_.PCI0.RP01.PXSX._ADR, UnknownObj)    // (from opcode)
    External (_TZ_.TZS0._TMP, MethodObj)    // 0 Arguments (from opcode)
    ...
    {
        OperationRegion (VBOR, SystemMemory, 0x7F791018, 0x00040004)
    ...
    Scope (\_SB.PCI0.RP01.PXSX)
    {
        OperationRegion (SGOP, SystemMemory, 0x7F10DF18, 0x00000046)
    ...
    Scope (\_SB.PCI0.RP01.PXSX)
    {
        OperationRegion (NOPR, SystemMemory, 0x7F78E018, 0x00002018)
        ...
        Method (GPS, 4, Serialized)
        {
            ...
            Switch (ToInteger (Arg2))
            {
                ...
                Case (0x20)
                {
                    ...
                    CreateField (GB32, 0x17, One, CPC2)
                    ...
                    Switch (ToInteger (QUTP))
                    {
                        Case (Zero)
                        {
                            If (CPUT)
                            {
                                Store (0x0200, STSV)
                                Or (STSV, QUTP, STSV)
                                Store (\_TZ.TZS0._TMP (), Local0)
                                Subtract (Local0, 0x0AAC, Local0)
                                Store (Divide (Local0, 0x0A, ), PDTS)
                            }
    ...
    Scope (\_SB.PCI0.RP01.PXSX)
    {
        ...
        {
            Store (One, \_SB.PCI0.LPCB.EC0.GCTM)
            Sleep (0x0A)
            Store (LREN, LTRE)
            Store (One, LNKD)
            While (LNotEqual (\_SB.PCI0.RP01.PXSX.SGPI (0x82020015), One))
            {
                Sleep (One)
            }
        }
        ...
        {
            Store (Zero, LNKD)
            \_SB.PCI0.RP01.PXSX.SGPO (0x02020014, One)
            While (LNotEqual (\_SB.PCI0.RP01.PXSX.SGPI (0x82020015), Zero))
            {
                Sleep (One)
            }

            \_SB.PCI0.RP01.PXSX.SGPO (0x02020014, Zero)
            Store (Zero, \_SB.PCI0.LPCB.EC0.GCTM)
            While (LLess (LNKS, 0x07))
            {
                Sleep (One)
            }
        ...
        {
            If (LEqual (\_SB.PCI0.RP01.PXSX.SGPI (0x82020015), Zero))
            {
                Return (One)
            }
            Else
